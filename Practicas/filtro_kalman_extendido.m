...
%% Parámetros
e = 1; %Error de modelado
a = 0.0005*e; K1 = 0.047*e; K2 = 0.065*e; A1 = 0.45*e; A2 = 0.45*e;
...
for k = 0:n-1
    ...
    x_pre = [x1 - (K1*T/A1)*sqrt(x1-x2) + a*T*uexp(k+1)/A1;
                x2 + K1*T*sqrt(x1-x2)/A2 - K2*T*sqrt(x2)/A2];

    Fi = [1 - (K1*T)/(2*A1*sqrt(x1-x2)), K1*T/(2*A1*sqrt(x1-x2)); 
        -K1*T/(2*A2*sqrt(x1-x2)), 1 + K1*T/(2*A2*sqrt(x1-x2)) - K2*T/(2*A2*sqrt(x2))];
...
%% Residuos
figure(7), plot(t, yexp' - C*x_est), title('Residuos x2')